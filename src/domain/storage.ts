import { createDefaultAppData, createDefaultDailySchedule } from './defaultData';
import { isValidScheduleTime, sortScheduleItems } from './dailySchedule';
import type {
  AppData,
  BacklogItem,
  DailyPomodoroPlan,
...
const CURRENT_APP_DATA_VERSION = 8;
const BACKUP_FORMAT = 'pomodoro-todo-app-backup';
...
export type AppDataExportEnvelope = {
...
function migratePreset(preset: Partial<TimerPreset>): TimerPreset {
  return {
    ...
    name: typeof preset.name === 'string' ? preset.name : '番茄',
...
function migrateDailyScheduleItem(value: unknown, index: number): DailyScheduleItem | null {
  if (!isRecord(value) || !isValidScheduleTime(value.startTime)) return null;
  const title = typeof value.title === 'string' ? value.title.trim() : '';
  if (!title) return null;

  return {
    id: typeof value.id === 'string' && value.id ? value.id : `schedule-imported-${index}-${crypto.randomUUID()}`,
    startTime: value.startTime,
    endTime: value.endTime === null || value.endTime === '' ? null : isValidScheduleTime(value.endTime) ? value.endTime : null,
    title,
...
function migrateDailySchedule(value: unknown): DailyScheduleSettings {
  const defaults = createDefaultDailySchedule();
  if (!isRecord(value)) return defaults;

  const sortedItems = Array.isArray(value.items)
    ? sortScheduleItems(
        value.items.map(migrateDailyScheduleItem).filter((item): item is DailyScheduleItem => item !== null)
      )
    : defaults.items;
  const enabledStartTimes = new Set<string>();
  const items = sortedItems.filter((item) => {
    if (!item.enabled) return true;
    if (enabledStartTimes.has(item.startTime)) return false;
    enabledStartTimes.add(item.startTime);
    return true;
  ...
  return {
    ...
    desktopNotificationEnabled:
      typeof value.desktopNotificationEnabled === 'boolean'
        ? value.desktopNotificationEnabled
        : defaults.desktopNotificationEnabled,
    autoLaunch: typeof value.autoLaunch === 'boolean' ? value.autoLaunch : defaults.autoLaunch,
    items
  };
}
...
function ensureAchievementTodosHaveTypeTag(todos: Todo[], typeTags: TodoTypeTag[]) {
  ...
  const needsItTag = todos.some(
    (todo) =>
      ...
      !todo.typeTagIds.some((tagId) => validTagIds.has(tagId))
  );
  ...
  return {
    ...
    todos: todos.map((todo) => {
      if (todo.status !== 'completed' && todo.checkInDates.length === 0) return todo;
...
function migrateAppData(value: StoredAppData): AppData | null {
  if (
    (value.version !== undefined && !isSupportedAppDataVersion(value.version)) ||
  ...
  const completedTagMigration = ensureAchievementTodosHaveTypeTag(value.todos.map(migrateTodo), typeTags);
...
export function parseAppDataExportEnvelope(raw: string): AppDataExportEnvelope | null {
  try {
    const parsed: unknown = JSON.parse(raw);
    if (
      !isRecord(parsed) ||
      parsed.format !== BACKUP_FORMAT ||
      !isSupportedAppDataVersion(parsed.version) ||
      typeof parsed.exportedAt !== 'string' ||
      Number.isNaN(Date.parse(parsed.exportedAt)) ||
      !isRecord(parsed.data) ||
      !isSupportedAppDataVersion(parsed.data.version) ||
      parsed.version !== parsed.data.version
    ) {
      return null;
    }
    const data = migrateAppData(parsed.data as StoredAppData);
    if (!data) return null;
    return {
...
export function parseAppDataExport(raw: string): AppData | null {
  return parseAppDataExportEnvelope(raw)?.data ?? null;
}

function isSupportedAppDataVersion(value: unknown): value is number {
  return (
    typeof value === 'number' &&
    Number.isInteger(value) &&
    value >= 1 &&
    value <= CURRENT_APP_DATA_VERSION
  );
}

export function serializeAppDataExport(data: AppData, exportedAt = new Date()) {
